...
public:
	...
	{
		if (p && getHashBin_fp) return ((p->*getHashBin_fp)(n)); else this->func_redefined = false; return 0;
	}
	bool isAccessible(NodeType& n) 
	{
		if (p && isAccessible_fp) return ((p->*isAccessible_fp)(n)); else this->func_redefined = false; return false;
	}
	...
	{
		if (p && getHeuristics_fp) return ((p->*getHeuristics_fp)(n1, n2)); else this->func_redefined = false; return NULL;
	}
	bool storePath(NodeType& n) 
	{
		if (p && storePath_fp) return ((p->*storePath_fp)(n)); else this->func_redefined = false; return false;
	}
	bool stopSearch(NodeType& n) 
	{
		if (p && stopSearch_fp) return ((p->*stopSearch_fp)(n)); else this->func_redefined = false; return false;
	}
};
